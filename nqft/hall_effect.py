...
class Model:
    ...
    mus: np.array, size=N, default=np.arange(-4, 4, 0.02)
        Chemical potential interval values and number of them.
    ...
    def __init__(self, hoppings: tuple[float], broadening: float, omega=0.0,
                 mus=arange(-4, 4, 0.02), resolution=400,
                 use_peters=False) -> None:
        """Docs
        """
        self.w = omega
        ...
        if use_peters:
            ...
            self.E, self.dEs = dE(hoppings, self.k_x, self.k_y, mus)
            ...
            self.E, self.dEs = dE(hoppings, self.k_x, self.k_y, mus)
            self.A = get_spectral_weight(omega, broadening, self.E)
